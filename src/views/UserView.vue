...
<script>
import { callGetUser, callEditUser, callDeleteUser } from "@/service/user";
...
export default {
  ...
  methods: {
    ...
    async modify() {
      ...
      const response = await callEditUser({
        ...this.user,
        newPwd: this.user.newPwd === "" ? null : this.user.newPwd,
      });

      if (
        response.status === this.HTTP_OK ||
        response.status === this.HTTP_CREATED
      ) {
        alert("수정 되었습니다.");
        this.refreshUser();
